...
function playSfx(type){
  try{
    if(window.__kairosMuted) return;
...
/* ---------- core scheduling logic, shared by the real app and the Test Command Center ---------- */
...
function currentLevel(xp){
  ...
  for(const l of AVTAARA_LEVELS){ if(xp>=l.min) level = l; }
...
function Logo({size=40, interactive=false}){
  ...
  return (
    <div className="logo-mark" ref={wrapRef} onMouseMove={handleMove} onMouseLeave={handleLeave}
         style={{width:size,height:size, perspective:interactive?400:undefined}}>
      <div className="logo-glow"/>
      <svg width={size} height={size} viewBox="0 0 100 100"
           style={{position:"relative", transform:interactive?`rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`:undefined, transition:"transform .12s ease-out"}}>
        <defs>
        ...
        <g className="logo-pulse">
          <path d="M50 20 L50 46 M50 54 L50 80 M32 32 L44 44 M56 56 L68 68 M32 68 L44 56 M56 44 L68 32"
...
function Atmosphere({kind}){
  ...
  useEffect(()=>{
    function onMove(e){
      ...
      if(farRef.current) farRef.current.style.transform = `translate(${nx*-6}px, ${ny*-6}px)`;
      if(midRef.current) midRef.current.style.transform = `translate(${nx*-16}px, ${ny*-16}px)`;
      if(nearRef.current) nearRef.current.style.transform = `translate(${nx*-32}px, ${ny*-32}px) scale(1.02)`;
    }
...
const THEME_SPARK = {
  cosmic:"✦", rain:"💧", aurora:"❄", zen:"○", grid:"▮", sunset:"☀", storm:"⚡", nebula:"✧",
};
...
function Avatar({color,color2,big,pulseKey,glyph,theme,onClick}){
  ...
  return (
    <div className={"avatar"+(big?" big":"")} style={{"--ac":color, "--ac2":color2||color, cursor:onClick?"pointer":undefined}} key={pulseKey} onClick={onClick}>
...
function Clock3D({size=150}){
  ...
  return (
    <div style={{width:size,height:size,perspective:600,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:size,height:size,position:"relative",transformStyle:"preserve-3d",transform:"rotateX(8deg)"}}>
        <svg width={size} height={size} viewBox="0 0 200 200" style={{filter:"drop-shadow(0 18px 24px rgba(0,0,0,0.45))"}}>
          ...
          {ticks.map(i=>(
            <line key={i} x1="100" y1="22" x2="100" y2={i%3===0?"32":"28"}
                  stroke={i%3===0?"var(--accent2)":"var(--muted)"} strokeWidth={i%3===0?"3":"1.5"}
                  transform={`rotate(${i*30} 100 100)`}/>
          ))}
          <line x1="100" y1="100" x2="100" y2="58" stroke="var(--text)" strokeWidth="5" strokeLinecap="round"
                transform={`rotate(${hourAngle} 100 100)`}/>
          <line x1="100" y1="100" x2="100" y2="38" stroke="var(--text)" strokeWidth="3.5" strokeLinecap="round"
                transform={`rotate(${minAngle} 100 100)`}/>
          <line x1="100" y1="112" x2="100" y2="30" stroke="var(--accent2)" strokeWidth="1.6" strokeLinecap="round"
                style={{transition:"transform 0.15s var(--ease-measure)"}}
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
      <svg width={size} height={size*1.3} viewBox="0 0 90 117" style={{filter:`drop-shadow(0 10px 18px rgba(0,0,0,0.4))${closingSoon?" drop-shadow(0 0 10px rgba(226,96,122,0.5))":""}`}}>
        ...
        <path d="M14 6 H76 V16 L48 58 L76 100 V110 H14 V100 L42 58 L14 16 Z"
              fill="url(#glassGrad)" stroke="var(--panel-border)" strokeWidth="2" strokeLinejoin="round"/>
        ...
        {grains.map((g,i)=>(
          <circle key={i} r="1.4" fill="var(--gold)" opacity="0.9">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:10}}>
        {WORLD_CLOCK_ZONES.map(z=>{
          ...
          try{
            ...
            if(zoneDay>localDay || (zoneDay===1 && localDay>=28)) dayDelta = "+1d";
            else if(zoneDay<localDay || (zoneDay>=28 && localDay===1)) dayDelta = "-1d";
          }catch(e){}
          return (
            <div key={z.tz} style={{textAlign:"center",padding:"8px 6px",borderRadius:10,background:"rgba(255,255,255,0.03)"}}>
...
function OpeningSequence({onDone}){
  ...
  return (
    <div className={"opening-sequence"+(stage>=4?" opening-exit":"")} onClick={onDone}>
      ...
      <div className={"opening-wordmark"+(stage>=3?" opening-wordmark-show":"")}>
        {OPENING_LETTERS.map((l,i)=>(
          <span key={i} className={"opening-letter"+(l==="O"?" opening-letter-o":"")} style={{animationDelay:(i*0.11)+"s"}}>{l}</span>
        ))}
      </div>
    ...
    <div className="center-screen">
      ...
      <div className="card login-card fade-in">
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,marginBottom:22}}>
          ...
          <div className="wordmark" style={{fontSize:24}}>KAIROS</div>
        ...
        <div className="role-row">
          {Object.entries(ROLES).map(([key,r])=>(
            <div key={key} className={"role-chip"+(role===key?" active":"")} onClick={()=>setRole(key)}>{r.label}</div>
          ))}
        </div>
...
const NAV_SECTIONS = [
  {label:"Studio", items:[["command","Command Center"],["calendar","Calendar"],["content","Content Studio"],["vibe","Vibe Lab"]]},
...
function Sidebar({page,setPage,session,onOpenPalette,notifications,unreadCount,onOpenNotifications,archetypeCtx}){
  ...
  return (
    <div className="sidebar card" style={{borderRadius:0}}>
      <div style={{display:"flex",alignItems:"center",gap:10,justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <Logo size={34}/>
          <div className="wordmark" style={{fontSize:15}}>KAIROS</div>
        </div>
        ...
        {NAV_SECTIONS.map((sec,i)=>(
          <div key={i}>
            ...
            {sec.items.map(([key,label])=>(
              <div key={key} className={"navitem"+(page===key?" active":"")} onClick={()=>{ playSfx("nav"); setPage(key); }}>
                <span>{label}</span>
              </div>
            ))}
          </div>
      ...
      {greetOpen &&
        <AvatarGreetingModal role={session.role} roleColor={role.color} roleColor2={role.color2}
                             theme={THEMES[session.theme].kind} archetypeCtx={archetypeCtx} onClose={()=>setGreetOpen(false)}/>}
    </div>
  );
}

/* ---------- command center (default + skins) ---------- */
...
function MobiusNavigator({setPage}){
  ...
  if(t<33){ zone="FUTURE"; targetPage="river"; zoneDesc="What hasn't happened yet."; }
  else if(t>66){ zone="MEMORY"; targetPage="timemachine"; zoneDesc="What's already layered beneath you."; }
  return (
  ...
  return (
    <div className="fade-in">
      ...
      <div className="stagger" style={{animationDelay:"0.19s",marginBottom:"var(--space-4)"}}>
        <DailyChallengeCard activityLog={activityLog||[]} claimed={challengeClaimed}
          onClaim={(challenge)=>{ if(onClaimChallenge) onClaimChallenge(challenge); }}/>
      </div>

      <div className="cc-row stagger" style={{animationDelay:"0.26s"}}>
        <FocusTimer onComplete={()=>{ playSfx("success"); if(bump) bump("Completed a 25-minute focus session", 8); }}/>
        <WorldClock/>
      </div>
    </div>
  );
...
function CommandCenterMinimal({session,setPage}){
  ...
  return (
    <div className="fade-in mono" style={{maxWidth:720}}>
      ...
      <div style={{marginTop:26,borderTop:"1px solid var(--panel-border)"}}>
        {links.map(([label,key])=>(
          <div key={key} style={{display:"flex",justifyContent:"space-between",padding:"14px 2px",borderBottom:"1px solid var(--panel-border)",cursor:"pointer"}} onClick={()=>setPage(key)}>
  ...
  return (
    <div className="fade-in">
      ...
      <div className="card galaxy-box">
        {bgStars.map((s,i)=>(
          <div key={i} className="galaxy-bg-star" style={{top:s.top+"%",left:s.left+"%",width:s.size,height:s.size,animationDelay:s.delay+"s",opacity:0.5}}/>
        ))}
        <svg className="galaxy-lines" width="100%" height="100%" style={{position:"absolute",inset:0}}>
          {links.map(([a,b],i)=>(
            <line key={i} x1={a.x+"%"} y1={a.y+"%"} x2={b.x+"%"} y2={b.y+"%"}
                  stroke={a.color} strokeWidth="1" strokeOpacity="0.35" strokeDasharray="2 5"/>
          ))}
        </svg>
        {positioned.map(e=>(
          <div key={e.id}
               ...
               onMouseLeave={()=>setHovered(h=>h&&h.id===e.id?null:h)}
               onClick={()=>setActive(e)}/>
        ))}
        {hovered &&
          <div style={{
...
function CommandCenterObservatory({session,setPage}){
  ...
  return (
    <div className="fade-in mono" style={{border:"1px solid var(--panel-border)",borderRadius:16,padding:26,background:"rgba(0,0,0,0.25)",textAlign:"center"}}>
...
function CommandCenterArchive({session,setPage}){
  return (
    <div className="fade-in archive-texture" style={{border:"1px solid var(--panel-border)",borderRadius:16,padding:28}}>
...
function CommandCenterClockwork({session,setPage}){
  return (
    <div className="fade-in" style={{border:"1px solid var(--panel-border)",borderRadius:16,padding:28,position:"relative",overflow:"hidden"}}>
...
function CommandCenterCyber({session,setPage}){
  ...
  return (
    <div className="fade-in mono scanlines" style={{border:"1px solid var(--panel-border)",borderRadius:14,padding:22,background:"rgba(0,0,0,0.35)"}}>
      ...
      <div style={{marginTop:20,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:10}}>
        {links.map(([label,key])=>(
          <div key={key} style={{border:"1px solid var(--panel-border)",borderRadius:8,padding:"10px 12px",cursor:"pointer",color:"var(--accent2)"}} onClick={()=>setPage(key)}>
            [{label}]
          </div>
        ))}
      </div>
...
function CommandCenter({session,setPage,skin,events,bump,activityLog,onClaimChallenge,challengeClaimed}){
  if(skin==="minimal") return <CommandCenterMinimal session={session} setPage={setPage}/>;
  if(skin==="galaxy") return <CommandCenterGalaxy session={session} setPage={setPage} events={events}/>;
  if(skin==="cyber") return <CommandCenterCyber session={session} setPage={setPage}/>;
  if(skin==="observatory") return <CommandCenterObservatory session={session} setPage={setPage}/>;
  if(skin==="celestial") return <CommandCenterCelestial session={session} setPage={setPage} activityLog={activityLog}/>;
  if(skin==="archive") return <CommandCenterArchive session={session} setPage={setPage}/>;
  if(skin==="clockwork") return <CommandCenterClockwork session={session} setPage={setPage}/>;
    ...
    <div className="fade-in">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:16}}>
        {options.map(([key,label,desc])=>(
          <div key={key} className="card" style={{padding:20,cursor:"pointer",borderColor: skin===key?"var(--accent)":undefined}} onClick={()=>setSkin(key)}>
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {months.map((m,i)=>{
        ...
        return (
          <g key={m} style={{cursor:"pointer"}} onClick={()=>onSelectMonth(i)}>
            <circle cx={x} cy={y} r={active?7.5:4} fill={active?"var(--gold)":"var(--panel-border)"}
                    style={{transition:"r .3s var(--ease-recurrence, ease), fill .3s var(--ease-recurrence, ease)"}}/>
...
function Calendar({events, onAdd, onRemove, onMove, maDays, setMaDays}){
  ...
  function addEvent(day){
    ...
    setDraftTitle("");
    setMaDays(prev=>{ if(!prev.has(day)) return prev; const next=new Set(prev); next.delete(day); return next; });
    setModalDay(null);
  }
    ...
    <div className="page-slide">
        ...
        <div className="card" style={{padding:"16px 20px",marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12}}>
            {freeSlots.map(slot=>{
              ...
              return (
                <div key={slot.day} style={{padding:"12px 14px",borderRadius:10,border:"1px solid var(--panel-border)",background:"rgba(255,255,255,0.03)"}}>
      ...
      <div className="cal-grid">
        {cells.map((day,i)=>{
          ...
          return (
            <div key={i}
                 className={"cal-cell"+(isToday?" today":"")+(dragOverDay===day?" dragover":"")}
                 onDragOver={e=>{e.preventDefault(); setDragOverDay(day);}}
                 onDragLeave={()=>setDragOverDay(prev=>prev===day?null:prev)}
                 onDrop={e=>{e.preventDefault(); handleDrop(day);}}
                 onClick={()=>{ if(dayEvents.length===0) setModalDay(day); }}>
              ...
              {dayEvents.map(ev=>(
                <div key={ev.id}
                     className={"event-chip "+chronoClass}
                     draggable
                     style={{background:ev.color}}
                     onDragStart={()=>sessionStorage.setItem("kairos-drag-id",ev.id)}>
                  <span>{ev.title}</span>
      ...
      {modalDay!==null &&
        <div className="modal-backdrop" onClick={()=>setModalDay(null)}>
          <div className="card modal" onClick={e=>e.stopPropagation()}>
            ...
            <input type="text" autoFocus placeholder="Event title" value={draftTitle}
                   onChange={e=>setDraftTitle(e.target.value)}
                   onKeyDown={e=>{ if(e.key==="Enter") addEvent(modalDay); }}/>
...
/* ---------- scheduler duel (calendar + all four concepts, live) ---------- */

function computeEventsByDay(events, totalDays){
  const map = {};
  for(let d=1; d<=totalDays; d++) map[d] = events.filter(e=>e.day===d);
  return map;
}
...
const DayCellInner = function DayCellInner({day, isToday, list, isDragOver, cellRef, onDragEnterDay, onDropDay, onRemoveDay, onChipDragStart, statsRef}){
  ...
  return (
    <div ref={cellRef}
         className={"cal-cell sched-cell"+(isToday?" today":"")+(isDragOver?" dragover":"")+(flash?(isZombie?" flash-zombie":" flash-render"):"")}
         data-day={day}
         onDragOver={e=>e.preventDefault()}
         onDragEnter={()=>onDragEnterDay(day)}
         onDrop={e=>{e.preventDefault(); onDropDay(day);}}>
      <span className="sched-rc">{isZombie?"👻 ":""}r:{renders.current}</span>
      <div className={"cal-daynum"+(isToday?" today":"")}>{day}</div>
      {list.map(ev=>(
        <div key={ev.id} className="event-chip" draggable style={{background:ev.color}}
...
function SchedulerDuel({events, onAdd, onRemove, onMove, onOptimizerUnlocked}){
  ...
  function setAllOptimizations(on){
    setMemoOn(on); setStructuralOn(on); setCallbackOn(on); setDragRefOn(on);
  }
  ...
  useEffect(()=>{
    if(memoOn && structuralOn && callbackOn && dragRefOn && !unlockedOnceRef.current){
      ...
      if(onOptimizerUnlocked) onOptimizerUnlocked();
    }
  ...
  function finishDrag(){
    ...
    Object.values(cellRefs.current).forEach(el=>{ if(el) el.classList.remove("dragover"); });
    setDragOverDay(null);
  ...
  const stableOnDragEnter = useCallback((day)=>{
    if(dragRefOn){
      if(hoveredRef.current !== day){
        ...
        if(prevEl) prevEl.classList.remove("dragover");
  ...
  function handleDragEnd(){
    // Fallback for drags that end without a valid drop (e.g. dropped outside
    // any cell). Successful drops are already finalized in onDropDay itself,
    // since the dropped chip's DOM node can be unmounted (moved to a new day)
    // before this dragend ever gets a chance to bubble up.
    finishDrag();
  }
  ...
  return (
    <div className="page-slide" style={{position:"relative"}} onDragEnd={handleDragEnd}>
      ...
      <div className="topbar">
        ...
        <div className="card" style={{padding:"10px 16px",display:"flex",gap:18,alignItems:"center"}}>
          ...
          <div>
            <div className="stat-label">Total this session</div>
            <div className="stat-big" style={{fontSize:26}}>{statsRef.current.totalRenders}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 300px",gap:18,alignItems:"start"}}>
          ...
          <div className="cal-grid">
            {cells.map(day=>(
              <DayCell key={day}
                       ...
                       list={eventsByDay[day]||[]}
                       isDragOver={dragOverDay===day}
                       cellRef={getCellRefCallback(day)}
                       onDragEnterDay={onDragEnterDay}
                       onDropDay={onDropDay}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          {CONCEPTS.map(c=>{
            ...
            return (
              <div key={c.key} className={"concept-card"+(on?" on":"")}>
...
const BaselineCard = function BaselineCard({item, onSelect, statsRef}){
  ...
  return (
    <div className={"duel-card"+(flash?" flash-render":"")} onClick={()=>onSelect(item.id)}>
        ...
        <div className="card duel-panel base-panel">
          ...
          <div className="duel-cardlist" key={"base"+shuffleKey+resetKey}>
            {baseItems
              ...
              .map(item=>(
                <BaselineCard key={item.id} item={item} onSelect={()=>{}} statsRef={statsRef}/>
              ))}
          </div>
        </div>
...
function RenderTotals({statsRef}){
  const [,forceTick] = useState(0);
  return (
    <div className="card" style={{padding:20,marginTop:18,display:"flex",gap:36,alignItems:"center",flexWrap:"wrap"}}>
      <div>
    ...
    <div className="page-scan">
      ...
      <div className="card" style={{padding:22,marginBottom:16,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",fontSize:12.5,alignItems:"center",position:"relative"}}>
          {["State changes","React re-renders the owning component","Every child is called again by default","Unless memoized AND its props are reference-stable","Then React can skip that child entirely"].map((s,i,arr)=>(
            <Fragment key={i}>
...
function TestCommandCenter({onAllPassed}){
  ...
  async function runAll(){
    ...
    setResults(out);
    setRunning(false);
    if(onAllPassed && out.every(r=>r.pass)) onAllPassed();
  }
    ...
    <div className="fade-in">
      ...
      <div>
        {tests.map((t,i)=>{
          ...
          return (
            <div key={t.name} className={"test-row"+(r? (r.pass?" test-pass":" test-fail") : "")}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
  ...
  return (
    <div className="fade-in">
      ...
      <div className="card" style={{padding:20}}>
        ...
        {log.map(item=>(
          <div key={item.id} className="log-line">
...
function HeavyList({count}){
  ...
  return (
    <div style={{maxHeight:220,overflow:"auto",border:"1px solid var(--panel-border)",borderRadius:10,padding:8}}>
...
function ChaosLab({onSurvived}){
  ...
  function injectSlowRender(){
    ...
    let x = 0;
    while(performance.now()-start < 180){ x += Math.random(); }
  ...
  function injectRapid(){
    ...
    rapidRef.current = setInterval(()=>{
      n += 1;
      setRapidCount(n);
      if(n>=80){ clearInterval(rapidRef.current); setRapidActive(false); }
    }, 30);
  }
  ...
  function restore(){
    ...
    setRapidCount(0);
    setApiChaosLog([]);
    if(chaosInjectedRef.current && onSurvived) onSurvived();
    chaosInjectedRef.current = false;
  }

  return (
    <div className="page-scan" style={{position:"relative"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:16}}>
        ...
        <div className="card" style={{padding:20}}>
          ...
          {apiChaosLog.map(x=>(
            <div key={x.id} className="log-line" style={{marginTop:10}}>
    ...
    <div>
      <div style={{height:viewportHeight,overflow:"auto",border:"1px solid var(--panel-border)",borderRadius:10}} onScroll={e=>setScrollTop(e.target.scrollTop)}>
        <div style={{height:items.length*itemHeight, position:"relative"}}>
          {visible.map((it,i)=>(
            <div key={it} className="virt-row" style={{position:"absolute",top:(startIndex+i)*itemHeight,width:"100%"}}>
              row #{it+1}
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:8}}>
            {[100,1000,5000,20000].map(n=>(
              <button key={n} className={"btn"+(size===n?" selected":"")} onClick={()=>setSize(n)}>{n.toLocaleString()}</button>
            ))}
          </div>
        </div>
        <div className="toggle-row" style={{border:"none",padding:0}}>
...
function TimeMachine({history, onRestore}){
  ...
  return (
    <div className="page-rewind" style={{position:"relative"}}>
        ...
        <div className="card duel-panel">
          ...
          <div style={{maxHeight:420,overflow:"auto"}}>
            {history.map((h,i)=>{
              ...
              return (
                <div key={h.ts+"-"+i} className={"hist-row"+(i===selected?" active":"")} onClick={()=>setSelected(i)}
                     style={{boxShadow: i===selected ? "0 10px 24px -4px rgba(0,0,0,0.55)" : `0 ${2+historicalDepth*10}px ${6+historicalDepth*20}px rgba(0,0,0,${0.1+historicalDepth})`,
        ...
        <div className="card duel-panel">
          ...
          <div className="duel-cardlist">
            {snapshot.events.map(e=>(
              <div key={e.id} className="duel-card">
                <span>{e.title}</span>
      ...
      <div>
        <div className="mirror-wrap">
          <div className="mirror-panel">
            ...
            <div className="duel-cardlist">
              {snapshot.events.map(e=>(
                <div key={e.id} className="duel-card"><span>{e.title}</span><span className="rc">day {e.day}</span></div>
              ))}
          ...
          <div className="mirror-panel">
            ...
            <div className="duel-cardlist">
              ...
              {removedSinceThen.map(e=>(
                <div key={"gone-"+e.id} className="duel-card" style={{opacity:0.4}}>
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="card modal" style={{width:"min(560px,92vw)"}} onClick={e=>e.stopPropagation()}>
        ...
        <textarea value={caption} onChange={e=>setCaption(e.target.value)}
                  ...
                  style={{width:"100%",minHeight:110,background:"rgba(255,255,255,0.05)",border:"1px solid var(--panel-border)",
        ...
        <div style={{marginTop:16}}>
          ...
          {[["Hook",quality.hookScore,"var(--accent)"],["CTA",quality.ctaScore,"var(--accent2)"],["Length",quality.lengthScore,"var(--gold)"],["Tags",quality.tagScore,"#7dd3fc"],["Structure",quality.structureScore,"#e2607a"]].map(([label,val,color])=>(
  ...
  return (
    <div className="page-canvas" style={{position:"relative"}}>
      ...
      <div className="card" style={{padding:"14px 20px",marginBottom:18,display:"flex",gap:10,alignItems:"center"}}>
        <input type="text" placeholder="Capture a new idea, hook, or content angle…" value={ideaTitle}
      ...
      <div className="pipeline-board">
        {STAGES.map(stage=>{
          ...
          return (
            <div key={stage}
                 className={"pipeline-col"+(dragOverStage===stage?" dragover":"")}
              ...
              {items.map(item=>(
                <div key={item.id} className="pipeline-card" draggable
                     onDragStart={()=>sessionStorage.setItem("kairos-content-drag-id", item.id)}
                  ...
                  <div style={{display:"flex",gap:6,marginTop:8}}>
                    <button className="btn btn-ghost" style={{padding:"4px 8px",fontSize:10.5}}
                            onClick={(e)=>{e.stopPropagation(); content.deleteItem(item.id);}}>Delete</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {scheduleFor!==null &&
        <div className="modal-backdrop" onClick={()=>setScheduleFor(null)}>
          <div className="card modal" onClick={e=>e.stopPropagation()}>
            ...
            <input type="text" value={dayInput} onChange={e=>setDayInput(e.target.value)}
                   onKeyDown={e=>{ if(e.key==="Enter") confirmSchedule(); }}/>
    ...
    <div className="fade-in">
      ...
      <div className="card" style={{padding:20}}>
        ...
        <div className="drag-log" style={{maxHeight:220}}>
          {activity.log.slice(0,20).map(l=>(
            <div key={l.id} className="drag-log-line">{new Date(l.ts).toLocaleTimeString()} — {l.text} <span style={{color:"var(--gold)"}}>+{l.xp}xp</span></div>
          ))}
        ...
        <div className="river-stage">
          <div className="river-wrap river-wrap-3d">
            ...
            {Array.from({length:totalDays},(_,i)=>i+1).map(d=>(
              <div key={"tick"+d} className={"river-tick"+(d%7===0?" river-tick-week":"")}
            ...
            {nodes.map(n=>{
              ...
              return (
                <div key={n.id}
                     className={"river-node"+(n.shape==="diamond"?" diamond":"")}
                     style={{
...
function TreeRings({unlockedCount, total, levelName}){
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        {rings.map((ring,i)=>(
          <circle key={i} cx={center} cy={center} r={ring.r} fill="none"
                  stroke={ring.unlocked ? "var(--accent2)" : "var(--panel-border)"}
                  strokeWidth={ring.unlocked?1.8:1}
                  opacity={ring.unlocked ? Math.max(0.35, 0.9-i*0.06) : 0.35}
                  style={{transition:`stroke .5s var(--ease-recurrence, ease), opacity .5s var(--ease-recurrence, ease)`}}/>
        ))}
    ...
    <div className="fade-in">
      ...
      <div className="achv-grid">
        {ACHIEVEMENTS.map(a=>{
          ...
          return (
            <div key={a.key} className={"achv-card tilt"+(isUnlocked?" unlocked":" locked")}>
...
function KairosMomentOverlay({achievement, onDone}){
  ...
  return (
    <div className="kairos-moment-overlay" onClick={onDone}>
      <div style={{position:"relative"}}>
        {particles.map((p,i)=>(
          <div key={i} className="moment-particle" style={{"--px":p.px+"px","--py":p.py+"px",left:"50%",top:"50%",animationDelay:p.delay+"s"}}/>
        ))}
...
function LevelUpOverlay({levelUp, onDone}){
  ...
  return (
    <div className="kairos-moment-overlay level-up-overlay" onClick={onDone}>
      <div style={{position:"relative"}}>
        {particles.map((p,i)=>(
          <div key={i} className="moment-particle" style={{"--px":p.px+"px","--py":p.py+"px",left:"50%",top:"50%",animationDelay:p.delay+"s",background:"var(--accent2)"}}/>
        ))}
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 64 64">
      ...
      {Array.from({length:blades}).map((_,i)=>{
        ...
        return (
          <path key={i}
                d={`M${cx} ${cy} L${cx+outerR*Math.cos(a1)} ${cy+outerR*Math.sin(a1)} L${cx+outerR*Math.cos(a2)} ${cy+outerR*Math.sin(a2)} Z`}
...
function TemporalParticles({type, count=14, active}){
  ...
  return (
    <div style={{position:"absolute",inset:0,overflow:"hidden",pointerEvents:"none"}} aria-hidden="true">
      {particles.map((p,i)=>{
        if(type==="dust") return <div key={i} className="particle-dust" style={{left:p.left+"%",top:p.top+"%","--dx":p.dx,"--dy":p.dy,animationDuration:p.dur+"s",animationDelay:p.delay+"s"}}/>;
        if(type==="falling") return <div key={i} className="particle-falling" style={{left:p.left+"%",animationDuration:(p.dur*0.5)+"s",animationDelay:p.delay+"s"}}/>;
        if(type==="converge") return <div key={i} className="particle-converge" style={{left:"50%",top:"50%","--sx":p.sx,"--sy":p.sy,animationDelay:(i*0.03)+"s"}}/>;
        if(type==="thread") return <div key={i} className="particle-thread" style={{left:p.left+"%",top:p.top+"%","--len":p.len,animationDelay:(i*0.05)+"s"}}/>;
        return null;
      })}
    </div>
  );
...
function Ouroboros({size=26}){
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" style={{animation:"spin 16s linear infinite"}}
         title="Ouroboros — self-contained recurrence. The end feeds the beginning.">
  ...
  return (
    <div className="fade-in">
        ...
        <div className="theme-grid">
          {visibleThemes.map(([key,t])=>(
            <div key={key}
                 className={"theme-chip"+(theme===key?" active":"")}
                 style={{background:`linear-gradient(135deg, ${t.accent}, ${t.accent2})`}}
                 onClick={()=>{ setTheme(key); setSession(s=>({...s,theme:key})); }}>
              {t.label}
            </div>
          ))}
        </div>
      ...
      <div className="card" style={{padding:20}}>
        ...
        <div className="role-row">
          {Object.entries(ROLES).map(([key,r])=>(
            <div key={key} className={"role-chip"+(session.role===key?" active":"")} onClick={()=>setSession(s=>({...s,role:key}))}>{r.label}</div>
          ))}
        </div>
...
/* ---------- command palette, toasts, focus timer ---------- */
...
function ToastStack({toasts}){
  return (
    <div className="toast-stack">
      {toasts.map(t=>(
        <div key={t.id} className={"toast"+(t.popping?" toast-pop":"")} style={{borderColor: t.tone==="success"?"var(--accent2)":t.tone==="error"?"var(--danger)":undefined}}>
          {t.text}
        </div>
      ))}
    </div>
...
function NotificationBell({notifications, unreadCount, onOpen}){
  ...
  return (
    <div style={{position:"relative"}}>
      <div className="notif-bell" onClick={()=>{ setOpen(o=>!o); if(!open) onOpen(); }}>
        🔔
  ...
  return (
    <div className="palette-backdrop" onClick={onClose}>
      <div className="card palette-box" onClick={e=>e.stopPropagation()}>
        <input ref={inputRef} className="palette-input" placeholder="Type a command or search pages…"
               value={query} onChange={e=>{setQuery(e.target.value); setActiveIdx(0);}} onKeyDown={onKeyDown}/>
        <div className="palette-list">
          {filtered.length===0 && <div className="palette-empty">No matching commands.</div>}
          {filtered.map((c,i)=>(
            <div key={c.label} className={"palette-item"+(i===activeIdx?" active":"")}
                 onMouseEnter={()=>setActiveIdx(i)} onClick={()=>run(c)}>
              <span>{c.label}</span>
...
function FocusTimer({onComplete}){
    ...
    const id = setInterval(()=>{
      setRemaining(r=>{
        if(r<=1){
          clearInterval(id);
          setRunning(false);
          if(!doneRef.current){ doneRef.current = true; if(onComplete) onComplete(); }
          return 0;
        }
        return r-1;
      });
  ...
  function toggle(){
    if(remaining===0){ setRemaining(DURATION); doneRef.current=false; setRunning(true); }
    else setRunning(x=>!x);
  }
    ...
    <div className="card" style={{padding:20,display:"flex",gap:18,alignItems:"center"}}>
      <div className="timer-ring-wrap">
        <svg width="110" height="110" viewBox="0 0 110 110">
          ...
          <circle cx="55" cy="55" r={r} fill="none" stroke="var(--accent)" strokeWidth="6" strokeLinecap="round"
...
function chronoPeriod(hour){
  if(hour>=5 && hour<8) return {name:"Dawn", icon:"🌅", filter:"brightness(1.05) sepia(0.05) saturate(1.05)"};
  if(hour>=8 && hour<17) return {name:"Day", icon:"☀", filter:"none"};
  if(hour>=17 && hour<20) return {name:"Dusk", icon:"🌇", filter:"brightness(0.97) sepia(0.08) saturate(1.08)"};
...
function setupGlobalInteractions(){
  ...
  function onMouseMove(e){
    ...
    document.querySelectorAll(".tilt.tilt-active").forEach(el=>{
      if(el!==tiltTarget){ el.style.transform = ""; el.classList.remove("tilt-active"); }
    });
    if(tiltTarget){
...
function App(){
  ...
  const [showOpening,setShowOpening] = useState(()=>{
    try{ return !sessionStorage.getItem("kairos-opening-seen"); }catch(e){ return true; }
  });
  ...
  function setMaDays(updater){
    setMaDaysRaw(prev=>{
      ...
      if(next.size>prev.size) activity.bump("Left a day as Ma — unclaimed on purpose", 2);
      return next;
    });
  }
  ...
  if(showOpening){
    return <OpeningSequence onDone={()=>{
      try{ sessionStorage.setItem("kairos-opening-seen","1"); }catch(e){}
      setShowOpening(false);
    }}/>;
  }

  if(!session){
    return <Login onEnter={(s)=>{ setSession(s); applyTheme(s.theme); }}/>;
  }
  ...
  const commands = [
    ...
    ...Object.keys(THEMES).filter(k=>!THEMES[k].secret || flags.easterEggFound).map(k=>(
      {label:`Switch theme → ${THEMES[k].label}`, hint:"theme", action:()=>{ setSession(s=>({...s,theme:k})); applyTheme(k); }}
    )),
    ...content.items.map(it=>(
      {label:`Content: ${it.title}`, hint:STAGE_LABELS[it.stage], action:()=>setPage("content")}
    )),
    ...store.events.map(ev=>(
      {label:`Event: ${ev.title}`, hint:`day ${ev.day}`, action:()=>setPage("calendar")}
    )),
  ];
  ...
  return (
    <div className="shell">
      ...
      <Sidebar page={page} setPage={setPage} session={session} onOpenPalette={()=>setPaletteOpen(true)}
               notifications={toasts.notifications} unreadCount={toasts.unreadCount} onOpenNotifications={toasts.markAllRead}
               archetypeCtx={{
      ...
      <div className="main">
        {page==="command" && <CommandCenter session={session} setPage={setPage} skin={skin} events={store.events} bump={activity.bump}
                              activityLog={activity.log} challengeClaimed={challengeClaimed}
                              onClaimChallenge={(challenge)=>{ setChallengeClaimed(true); activity.bump(`Claimed daily challenge: ${challenge.label}`, challenge.xp); toasts.push(`+${challenge.xp} XP claimed`, "success"); }}/>}
